...
//   return (
//     <div className={`flex flex-col md:flex-row font-jakarta ${isDarkTheme ? 'dark' : 'light'}`}>
//       {/* En-tête mobile */}
...
//       {/* Overlay mobile */}
//       <div
//         className={`fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 backdrop-blur-sm z-20 ${isSidebarOpen ? 'block' : 'hidden'} md:hidden overlay-blur`}
...
export default function AgencyLayout({ children }: { children: React.ReactNode }) {
  ...
  // Gestion du thème (commenté pour éviter des problèmes potentiels, à réactiver si nécessaire)
  // useEffect(() => {
  //   const savedTheme = localStorage.getItem('theme');
  //   const prefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
  //   if (savedTheme === 'dark' || (!savedTheme && prefersDark)) {
...
//         toggleTheme={toggleTheme}
//         language={language}
//         setLanguage={setLanguage} // Prop ajoutée pour la langue
//       />
